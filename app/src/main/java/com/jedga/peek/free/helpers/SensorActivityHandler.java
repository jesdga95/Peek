...
public class SensorActivityHandler {
    ...
    private final static String TAG = "SensorActivityHandler";
    ...
    public SensorActivityHandler(Context context, SensorChangedCallback callback, boolean adjustMode) {
        mContext = context;
        mCallback = callback;
        ...
        if (!mAdjustMode) mScreenReceiver = new ScreenReceiver();
        ...
        if (mProximitySensor != null) {
            mProximityEventListener = new SensorEventListener() {
                ...
                public void onSensorChanged(SensorEvent event) {
                    ...
                    if (DEBUG) Log.d(TAG, "Received value: " + value);
                    if (mAdjustMode) {
                        mCallback.onProximityValueReceived(value);
                        return;
                    }
                    boolean inPocket = value == mProximityValue;
                    if (inPocket) {
        ...
        if (mGyroscopeSensor != null) {
            mGyroscopeEventListener = new SensorEventListener() {
                ...
                public void onSensorChanged(SensorEvent event) {
                    ...
                    if (DEBUG) Log.d(TAG, "Received values: x: " + x + " y: " + y + " z: " + z);
                    if (mAdjustMode) {
                        mCallback.onGyroscopeValuesReceived(x, y, z);
                        return;
                    }
                    ...
                    if (mHasInitialValues) {
                        ...
                        if (dX >= mMotionThreshold ||
                                dY >= mMotionThreshold || dZ >= mMotionThreshold) {
                            if (mWaitingForMovement) {
                                if (DEBUG) Log.d(TAG, "On table: false");
                                mOnTable = false;
                            ...
                            // we are assuming that device is on table for now.
                            if (mOnTable) {
                            ...
                            if (mSensorIncrement < INCREMENTS_TO_DISABLE) {
                                ...
                                if (mSensorIncrement == INCREMENTS_TO_DISABLE) {
                                    unregisterProximityEvent();
                                    if (DEBUG) Log.d(TAG, "On table: true");
                                    mOnTable = true;
                                    mCallback.onTableModeChanged(mOnTable);
                                    mWaitingForMovement = true;
                                }
    ...
    private void updateMotionThreshold() {
        mMotionThreshold = PreferencesHelper.getMotionThresholdValue(mContext);
        if (DEBUG) Log.i(TAG, "Motion threshold: " + mMotionThreshold);
    }

    private void updateProximityValue() {
        mProximityValue = PreferencesHelper.getProximityValue(mContext);
        if (DEBUG) Log.i(TAG, "Proximity value: " + mProximityValue);
    }

    public boolean isPhoneCharging() {
        final Intent batteryIntent = mContext
        ...
        boolean charging = plugged == BatteryManager.BATTERY_PLUGGED_USB ||
                plugged == BatteryManager.BATTERY_PLUGGED_AC ||
                status == BatteryManager.BATTERY_STATUS_CHARGING;
        if (DEBUG) Log.d(TAG, "Charging status: " + (charging ? "charging" : "discharging"));
        return charging;
    }
    ...
    public void registerProximityEvent() {
        if (!mProximityRegistered) {
            if (DEBUG) Log.d(TAG, "Registering proximity polling");
    ...
    public void registerGyroscopeEvent() {
        if (!mGyroscopeRegistered && (!PreferencesHelper.shouldWakeUp(mContext) || mAdjustMode)) {
            if (DEBUG) Log.d(TAG, "Registering gyroscope polling");
    ...
    public void unregisterProximityEvent() {
        if (mProximityRegistered) {
            if (DEBUG) Log.d(TAG, "Unregistering proximity polling");
    ...
    public void unregisterGyroscopeEvent() {
        if (mGyroscopeRegistered) {
            if (DEBUG) Log.d(TAG, "Unregistering gyroscope polling");
    ...
    public interface SensorChangedCallback {
        ...
        public abstract void onProximityValueReceived(float value);

        public abstract void onGyroscopeValuesReceived(float x, float y, float z);
    }
    ...
    public void resetToInitialPosition() {
        mInPocket = false;
        // let's assume phone is back on table, unless we wan't to wakeup
        mOnTable = !PreferencesHelper.shouldWakeUp(mContext);
    }
}